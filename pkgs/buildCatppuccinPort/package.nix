{ lib, stdenvNoCC, catppuccinInstallHook, fetchCatppuccinPort, sources, }:

args:

stdenvNoCC.mkDerivation (finalAttrs:

  let args' = if lib.isFunction args then args finalAttrs else args;

  in args' // {
    version = args'.version or (builtins.substring 0 7 finalAttrs.src.rev);

    src = args'.src or sources.${finalAttrs.pname} or (fetchCatppuccinPort {
    ...
    meta = {
      ...
      maintainers = with lib.maintainers; [ getchoo isabelroses ];
