...
in {
  options.catppuccin.waybar =
    catppuccinLib.mkCatppuccinOption { name = "waybar"; } // {
      mode = lib.mkOption {
        type = lib.types.enum [ "prependImport" "createLink" ];
  ...
  imports = (catppuccinLib.mkRenamedCatppuccinOptions {
    from = [ "programs" "waybar" "catppuccin" ];
    to = "waybar";
  }) ++ [
    (lib.mkRenamedOptionModule [ "programs" "waybar" "catppuccin" "mode" ] [
      "catppuccin"
      "waybar"
      "mode"
    ])
