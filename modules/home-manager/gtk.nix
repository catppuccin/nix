...
let
  inherit (lib)
    concatStringsSep mkEnableOption mkIf mkMerge mkOption mkRenamedOptionModule
    types;
...
in {
  options.catppuccin.gtk = catppuccinLib.mkCatppuccinOption {
    name = "gtk";
    ...
    tweaks = mkOption {
      type = types.listOf (types.enum [ "black" "rimless" "normal" "float" ]);
      default = [ ];
  ...
  imports = (catppuccinLib.mkRenamedCatppuccinOptions {
    ...
    from = [ "gtk" "catppuccin" "cursor" ];
    to = "cursors";
    accentSupport = true;
  }) ++ [
    (mkRenamedOptionModule [ "gtk" "catppuccin" "size" ] [
      "catppuccin"
      "gtk"
      "size"
    ])
    ...
    (mkRenamedOptionModule [ "gtk" "catppuccin" "gnomeShellTheme" ] [
      "catppuccin"
      "gtk"
      "gnomeShellTheme"
    ])

    (mkRenamedOptionModule [ "gtk" "catppuccin" "icon" ] [
      "catppuccin"
      "gtk"
      "icon"
    ])
  ...
  config = mkMerge [
    ...
    (mkIf enable {
      gtk.theme = let gtkTweaks = concatStringsSep "," cfg.tweaks;
      in {
        name = "catppuccin-${cfg.flavor}-${cfg.accent}-${cfg.size}+"
    ...
    (mkIf cfg.icon.enable {
      ...
      in {
        ...
        package = pkgs.catppuccin-papirus-folders.override {
          inherit (cfg.icon) accent flavor;
        };
      };
    })
    ...
    (mkIf cfg.gnomeShellTheme {
      assertions = [{
        assertion = enable;
        message =
          "`gtk.enable` and `gtk.catppuccin.enable` must be `true` to use the GNOME shell theme";
      }];
      ...
      dconf.settings = {
        ...
        "org/gnome/desktop/interface" = {
          color-scheme =
            if cfg.flavor == "latte" then "default" else "prefer-dark";
        };
      };
    })
  ];
