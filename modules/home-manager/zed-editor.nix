...
in {
  options.catppuccin.zed = catppuccinLib.mkCatppuccinOption {
    name = "zed";
    accentSupport = true;
  } // {
  ...
  config = lib.mkIf enable {
    programs.zed-editor = {
      userSettings.theme = {
        light = "Catppuccin " + catppuccinLib.mkUpper cfg.flavor + accent
          + lib.optionalString (!cfg.italics) " - No Italics";
        dark = "Catppuccin " + catppuccinLib.mkUpper cfg.flavor + accent
          + lib.optionalString (!cfg.italics) " - No Italics";
      };
    };
