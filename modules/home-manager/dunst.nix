...
in {
  options.catppuccin.dunst =
    catppuccinLib.mkCatppuccinOption { name = "dunst"; } // {
      prefix = lib.mkOption {
        type = lib.types.str;
        default = "00";
  ...
  imports = (catppuccinLib.mkRenamedCatppuccinOptions {
    from = [ "services" "dunst" "catppuccin" ];
    to = "dunst";
  }) ++ [
    (lib.mkRenamedOptionModule [ "services" "dunst" "catppuccin" "prefix" ] [
      "catppuccin"
      "dunst"
      "prefix"
    ])
  ...
  config = lib.mkIf enable {
    xdg.configFile = {
      ...
      "dunst/dunstrc.d/${cfg.prefix}-catppuccin.conf".source = sources.dunst
        + "/${cfg.flavor}.conf";
    };
  };
}
