...
in {
  options.catppuccin.sddm = catppuccinLib.mkCatppuccinOption { name = "sddm"; }
    // {
      ...
      assertQt6Sddm = lib.mkEnableOption ''
  ...
  imports = (catppuccinLib.mkRenamedCatppuccinOptions {
    from = [ "services" "displayManager" "sddm" "catppuccin" ];
    to = "sddm";
  }) ++ [
  ...
  config = lib.mkIf enable {
    assertions = lib.optional cfg.assertQt6Sddm {
      assertion = config.services.displayManager.sddm.package
    ...
    services.displayManager = {
      sddm = { theme = "catppuccin-${cfg.flavor}"; };
    };
