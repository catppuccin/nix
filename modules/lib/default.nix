...
let
  inherit (lib)
    flip importJSON mkEnableOption mkOption mkRenamedOptionModule
    mkSinkUndeclaredOptions optional optionalAttrs stringLength substring
    toUpper types versionAtLeast;

  inherit (lib.modules) importApply;

  inherit (pkgs) runCommand;

in lib.makeExtensible (ctp: {
  types = {
    flavor = types.enum [ "latte" "frappe" "macchiato" "mocha" ];
  ...
  */
  mkCatppuccinOption = { name, useGlobalEnable ? true
    ...
    {
      enable = mkEnableOption "Catppuccin theme for ${name}" // ({
        inherit default;
  ...
  */
  getModuleRelease =
    config.home.version.release or config.system.nixos.release or (throw
      "Couldn't determine release version!");
  ...
  */
  mkVersionedOpts = minVersion: options:
    if versionAtLeast ctp.getModuleRelease minVersion then
      options
    else
  ...
  */
  assertMinimumVersion = version: {
    ...
    message =
      "`catppuccin/nix` requires at least version ${version} of NixOS/home-manager";
  };
  ...
  */
  ...
  /* *
     ...
     ```
  */
  mkRenamedCatppuccinOptions = { from, to, accentSupport ? false, }:
    [
      (mkRenamedOptionModule (from ++ [ "enable" ]) [
        "catppuccin"
        to
        "enable"
      ])

      (mkRenamedOptionModule (from ++ [ "flavor" ]) [
        "catppuccin"
        to
        "flavor"
      ])
    ] ++ optional accentSupport
    (mkRenamedOptionModule (from ++ [ "accent" ]) [ "catppuccin" to "accent" ]);
})
